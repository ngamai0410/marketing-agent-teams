...
>
> **Added June 13, 2026 — not in original roadmap (end-user request):** a **live monitoring dashboard + per-step QC gates**. New `core/reporter.py` (passive metrics bus: per-agent calls/tokens/$/searches/elapsed, async pub/sub, `run_report.md` digest — `run_agent` taps it at start/call/search/done) and `core/checkpoint.py` (`await checkpoint()` — Approve / Edit brief & re-run / Quit; auto-approves headless). New `embroidery/web/` package (FastAPI + uvicorn + SSE, single vanilla HTML page) launched via `python -m embroidery.web`. Research `pipeline.py` rewired into a gate-driven loop (two gates: after A/B/C, after synthesis) — the reference pattern copy/QA/orchestrator adopt. Deps added: `fastapi`, `uvicorn[standard]`. Verified: imports, reporter metrics/cost, gate publish/resolve/auto-approve, server endpoints, deterministic gate-loop integration test, smoke + Agent 7 tests still pass.
>
> **Also June 13 — editable prompts (end-user request).** New `core/prompt_store.py`: the user can view/edit/save **each agent's system prompt** (3 sub-agents + shared rules + 2 Synthesizer prompts) from the dashboard's **⚙ Agent prompts** panel before Start. `to_dollar()` converts the `.format` templates (`{{}}`-escaped) to a brace-safe `$placeholder` form so JSON-heavy prompts edit cleanly; overrides persist to `data/prompts/overrides.json`; `build_system()` / `run_synthesizer()` render through the store (`Template.safe_substitute`). New endpoints `GET/POST /prompts`, `POST /prompts/reset` (locked while a run is in flight). Verified: `to_dollar` byte-exactly reproduces `.format` for all 5 system templates; override flow-through (incl. single-brace JSON + removed-placeholder safety); live endpoint round-trip.

> **June 13, 2026 — whole-team dashboard (phases 1–4, end-user request).** Four implementation phases that lift the dashboard from research-only to a generic, registry-driven team control surface. Spec: `docs/superpowers/specs/2026-06-13-web-team-dashboard-design.md`. Plan: `docs/superpowers/plans/2026-06-13-web-team-dashboard.md`.
>
> **Phase 1 — WorkflowSpec registry (`core/workflow.py`).** `Stage(name, agents, digest?)` + `WorkflowSpec(id, label, stages, entry_point, prompt_catalog, inputs, outputs, fixtures, config_schema)`. `register()` / `get_registry()` / `get_spec()` / `clear_registry()` (test helper). `load_workflows()` imports pipeline modules in canonical order (lazy + tolerant of not-yet-built ones). The web layer and orchestrator are now fully generic — adding a workflow = write its pipeline module + register.
>
> **Phase 2 — reporter + checkpoint workflow tagging.** `AgentRecord` gained a `workflow` field (first key in `as_row()`); `reporter.workflow_context(id)` contextmanager (contextvar) tags each `agent_start` row to its lane. `checkpoint()` signature extended: `await checkpoint(stage, digest, *, workflow="", request=None)` — carries `workflow` in the gate event and in `open_gates()`. Additive changes, no behaviour break.
>
> **Phase 3 — research pipeline retrofitted + QA wired.** `agents/research/pipeline.py` now registers `WorkflowSpec(id="research")` at import and wraps its body in `reporter.workflow_context("research")`; no longer publishes run-level done (orchestrator owns that). New `agents/qa/pipeline.py` registers `WorkflowSpec(id="qa")` (Agent 7 as a gated entry point) — the second wired workflow. `qa_reviewer.py` renders its system prompt via `prompt_store` (prompt catalog exposed). `web/server.py` calls `load_workflows()` at import; `/prompts` is now registry-driven across all registered workflows; three new endpoints added: `GET /workflows` (registry snapshot), `GET /artifacts` (list data/output files), `POST /prompts/preview` (dry-run prompt render with sample context).
>
> **Phase 4 — generic orchestrator + Test/Run UI.** New `core/orchestrator.py`: `run_team(brief, *, start, stop, start_stage, stop_stage, gate, max_qa_loops=2)` walks the registry `[start..stop]`; asserts each workflow's declared `inputs` exist under `data/output/` before running it (data-contract gate — `blocked` done event when files are missing, not a silent skip); runs each `entry_point` inside `reporter.workflow_context(id)`; after `qa` re-loops back to `copy` on overall FAIL (bounded by `max_qa_loops`); owns run-level `done`/`aborted`/`blocked` + `run_report.md`. `POST /start` now routes through `run_team` with `{target, start_stage, stop_stage, brief, seed_fixtures}` body. Dashboard UI: workflow **lanes** (one column per registered workflow, agent rows grouped) + **rail** header; **🧪 Test / Run** panel (target selector, stage range, seed-from-fixtures toggle, dry-run prompt preview).
>
> **Status after phases 1–4:** research + QA wired as distinct lanes; the full team run (`target="team"`) chains them through `run_team`; data-contract gating enforced at workflow boundaries; all three dashboard pillars (Monitor/Test/Edit) functional for the two wired workflows. **Phases 5–6 remain:** config-editing from the UI, Copy + Feedback workflows, cross-run persistence / CampaignStore.
